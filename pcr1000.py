...
def main(args):
    ...
    # start receiving
    pcr.start(volume=args.volume)

    # scan from 88Mhz to 105Mhz wideband FM in 10kHz intervals
    if args.band == "fm":
        start, stop, interval, sleep_time, mode, flt = int(88e6), int(105e6), 10000, 0.1, PCR1000.MODE_WFM, PCR1000.FLT_230K
    elif args.band == "am":
        start, stop, interval, sleep_time, mode, flt = int(600e3), int(1700e3), 200, 0.01, PCR1000.MODE_AM, PCR1000.FLT_6K
    ...
    for freq in range(start, stop, interval):
        print(freq)
        pcr.tune(freq, mode, flt)
        time.sleep(sleep_time)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--band", choices=["am", "fm"], default="fm", help="Band to scan (one of am, fm)")
    ...
    def parse_frequency(freq):
        ...
        try:
            ...
            if freq.endswith("m"):
                rv = int(freq[:-1]) * 1000000
            elif freq.endswith("k"):
                rv = int(freq[:-1]) * 1000
        return rv

    if args.start is not None:
        args.start = parse_frequency(args.start)
    if args.stop is not None:
        args.stop = parse_frequency(args.stop)
    if args.interval is not None:
        args.interval = parse_frequency(args.interval)
